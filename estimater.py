...
class FoundationPose:
    def __init__(
        ...
        self.make_rotation_grid(
            min_n_views=rotation_grid_min_n_views,
            inplane_step=rotation_grid_inplane_step,
        )
    ...
    def track_one(
        ...
        if init_ob_pos_center is not None:
            self.pose_last[:, :3, 3].data = init_ob_pos_center.clone()

        if prev_pose is not None:
            tf_to_centered_mesh = self.get_tf_to_centered_mesh().cpu().numpy()
            prev_pose = prev_pose @ np.linalg.inv(tf_to_centered_mesh)
        ...
        pose, vis = self.refiner.predict(
            ...
            K=K,
            ob_in_cams=(
                self.pose_last.reshape(1, 4, 4).data.cpu().numpy()
                if prev_pose is None
                else prev_pose.reshape(1, 4, 4)
            ),
            normal_map=None,
            xyz_map=xyz_map,
            mesh_diameter=self.diameter,
    ...
    def track_batch(
        ...
        tf_to_centered_mesh = self.get_tf_to_centered_mesh().cpu().numpy()
        poses = ob_in_cams @ np.linalg.inv(tf_to_centered_mesh)
        poses = torch.as_tensor(poses, device="cuda", dtype=torch.float)
        xyz_map = depth2xyzmap(depth, K)
        poses, vis = self.refiner.predict(
            mesh=self.mesh,
